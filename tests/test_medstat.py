...
def test_index_names(exp_1, exp_2, make_dataset):
    data = make_dataset(3)
    result = medstat.check(data, exp_1, exp_2)
...
def test_missing_boolean_category(make_dataset):
    data = make_dataset(5)
    result = medstat.check(data, 'q1', 'q2 > 10')
...
def test_contigency_table_values(make_dataset):
    ...
    result = medstat.check(data, 'q1 == 200', 'q2 >= 150')
    ...
    data = make_dataset(4)
    result = medstat.check(data, 'q1 == 1', 'q3')
...
def test_chisquared_above_10(make_dataset):
    data = make_dataset(300)
    result = medstat.check(data, 'q1 > 200', 'q3')
    assert result['test'] == "Chi-squared"


def test_fisher_below_10(make_dataset):
    data = make_dataset(300)
    result = medstat.check(data, 'q1 < 27', "q3 == 'a'")
    assert result['test'] == "Fisher"
    

def test_significant(make_dataset):
    data = make_dataset(300)
    result = medstat.check(data, 'q1 % 6 == 0', "q3 == 'a'")
    assert result['is-significant']
    assert pytest.approx(result['p-value'], 0.1) == 0


def test_not_significant(make_dataset):
    data = make_dataset(300)
    result = medstat.check(data, 'q1 < 100', "q3 == 'a'")
    assert not result['is-significant']
